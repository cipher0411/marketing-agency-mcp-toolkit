...
| 17 | Brevo | Remote MCP at `mcp.brevo.com/v1/brevo/mcp`, bridged via `mcp-remote` with a Bearer token header | Brevo (official, developers.brevo.com/docs/mcp-protocol) | Free-tier alternative to Klaviyo — historically ~300 emails/day free, unlimited contacts (verify current limits). **Account-specific bug found:** a first account's token completed OAuth successfully but got 401 on every real API call; a second, independently-created account's token connected cleanly with no issues, confirming this is per-account (plan/permission flag), not a defect in Brevo's MCP server itself. If a client's Brevo account hits this, try regenerating the key and confirming their plan includes API access before assuming it's broken |
| 18 | Buffer | Official Buffer MCP, connects via Claude.ai's account-level Settings → Connectors UI | Buffer (official, buffer.com/mcp) | Free-tier alternative to Social Neuron's scheduling/distribution piece. **Cannot be wired into `.mcp.json`** — confirmed against both buffer.com/mcp and developers.buffer.com, neither publishes a standalone remote MCP URL; it's Connector-only, same category as the `claude.ai HeyGen`/`claude.ai Kling ai` entries already in this account's `/mcp` list. Connect via claude.ai Settings → Connectors instead |
| 19 | Saleshandy | `ftaxats/SHMCP` on GitHub, self-hosted with a custom stdio wrapper | Individual community project | Cheaper alternative to Instantly (~$25/mo, 7-day trial). **The published repo doesn't run as a standard MCP server at all** — packaged for Smithery's cloud hosting only, and its compiled output was also missing required `.js` extensions on ESM imports (`ERR_MODULE_NOT_FOUND` on Node). Both fixed locally: patched the compiled imports and wrote a `stdio-entry.mjs` that calls the exported `createServer()` directly. Verified end-to-end (all 9 tools registered cleanly). Source reviewed — only calls the real `api.saleshandy.com`. |
| 20 | Smartlead | `smartlead-mcp-by-leadmagic` on npm | LeadMagic (official SmartLead partner) | Cheaper alternative to Instantly ($39/mo, 14-day trial, 2,500 email credits). Worked correctly with zero fixes on first runtime test: 49/113+ tools loaded by default, and it properly rejected a placeholder key with a real 401 instead of silently accepting bad input — the cleanest first-try result of any community/partner MCP tested this session. |
